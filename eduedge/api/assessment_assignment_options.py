...
def _capability_group_names(branch: str, reference_date) -> set[str]:
    ...
    if meta.has_field(OFFERING_FIELD):
        fields.append(OFFERING_FIELD)
    # Exact capability rows plus Branch Eligibility already authorize discovery.
    # Use an internal metadata read here so first Assessment Plan creation does not
    # depend on schedule-derived Student Group list visibility.
