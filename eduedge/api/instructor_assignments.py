...
def _all_options(allowed: list[dict]) -> tuple[list[dict], list[dict], list[dict], dict[str, set[str]]]:
    ...
    courses, course_map = core._course_map(offerings)
    return offerings, groups, courses, course_map
...
def get_instructor_assignments_page(
    ...
    offering_rows, groups, courses, course_map = _all_options(allowed)
...
def _plan(payload: dict) -> tuple[list[PlannedAssignment], list[PlannedBranchAccess], dict]:
    ...
    instructor_row = frappe.db.get_value("Instructor", instructor, ["name", "instructor_name", "status", INSTITUTION_FIELD], as_dict=True)
    ...
    for index, row in enumerate(rows, 1):
        ...
        if scope == BRANCH_ONLY_SCOPE:
            branch_access.append(PlannedBranchAccess(row_id, branch_name, institution, start, end, enabled, notes, f"{branch.get('institution_name') or institution} · {branch.get('branch_name') or branch_name}"))
        ...
        if period_start and start and getdate(start) < getdate(period_start):
            frappe.throw(
                _("{0}: Valid From cannot be earlier than the selected Class academic period.").format(label),
                frappe.ValidationError,
            )
        if period_end and end and getdate(end) > getdate(period_end):
            frappe.throw(
                _("{0}: Valid To cannot be later than the selected Class academic period.").format(label),
                frappe.ValidationError,
            )
        ...
        for course in courses:
            if course not in program_courses.get(offering.program, set()):
                frappe.throw(_("{0}: Subject / Course {1} is not configured for Class {2}.").format(label, frappe.db.get_value("Course", course, "course_name") or course, offering.offering_title or offering.name), frappe.ValidationError)
            course_institution = frappe.db.get_value("Course", course, INSTITUTION_FIELD)
        ...
        for group in targets:
            for course in courses or [None]:
                ...
                planned.append(PlannedAssignment(row_id, branch_name, institution, offering.name, group.name if group else None, course, scope, assignment_type, start, end, enabled, notes, " · ".join(str(value) for value in parts if value)))
                count += 1
        summaries.append({"row_id": row_id, "scope": scope, "record_count": count})
    # Invalid combinations are rejected; nothing is silently skipped.
    _validate_batch_duplicates(planned)
    _validate_branch_access_duplicates(branch_access)
    return planned, branch_access, {"instructor": instructor, "assignment_institutions": sorted(institutions), "row_summaries": summaries}


def _identity(row: PlannedAssignment) -> tuple:
    return (row.branch, row.program_offering, row.student_group or "", row.course or "", row.assignment_scope, row.assignment_type)
        ...
        for index, row in enumerate(rows):
            for other in rows[index + 1 :]:
                if core._overlap(row.valid_from, row.valid_to, other.valid_from, other.valid_to):
                    frappe.throw(_("Assignment Rows {0} and {1} produce the same overlapping academic responsibility. Merge or remove one row.").format(row.row_id, other.row_id), frappe.DuplicateEntryError)
    ...
    for branch_rows in grouped.values():
        for index, row in enumerate(branch_rows):
            for other in branch_rows[index + 1 :]:
                if row.enabled and other.enabled and core._overlap(
                    row.valid_from, row.valid_to, other.valid_from, other.valid_to
                ):
                    frappe.throw(
                        _("Assignment Rows {0} and {1} create overlapping explicit Branch access for {2}.").format(
                            row.row_id, other.row_id, row.branch
                        ),
...
def _classify(plan: list[PlannedAssignment], instructor: str) -> tuple[list, list, list]:
    ...
    for row in plan:
        ...
        matching = [value for value in records if (value.student_group or None) == row.student_group and (value.course or None) == row.course]
        exact = next((value for value in matching if core._same_date(value.valid_from, row.valid_from) and core._same_date(value.valid_to, row.valid_to)), None)
        if exact:
            existing.append({"name": exact.name, "row_id": row.row_id, "label": row.label, "enabled": cint(exact.enabled), "requested_enabled": row.enabled, "notes": row.notes})
            continue
        overlap = next((value for value in matching if cint(value.enabled) and row.enabled and core._overlap(row.valid_from, row.valid_to, value.valid_from, value.valid_to)), None)
        if overlap:
            conflicts.append({"name": overlap.name, "row_id": row.row_id, "label": row.label, "reason": _("Overlapping active assignment for this Instructor")})
        else:
            create.append(row)
...
def _primary_conflicts(plan: list[PlannedAssignment], instructor: str) -> list[dict]:
    ...
    for row in plan:
        ...
        records = frappe.get_all("EduEdge Instructor Assignment", filters=filters, fields=["name", "instructor", "valid_from", "valid_to"], limit_page_length=0)
        overlap = next((value for value in records if core._overlap(row.valid_from, row.valid_to, value.valid_from, value.valid_to)), None)
        if overlap:
            conflicts.append({"name": overlap.name, "row_id": row.row_id, "label": row.label, "reason": _("{0} already has another active primary Instructor.").format(row.assignment_type), "other_instructor": overlap.instructor})
    return conflicts


def _branch_periods(instructor: str, branch: str) -> list[dict]:
    return frappe.get_all("EduEdge Instructor Branch Assignment", filters={"instructor": instructor, "school_branch": branch}, fields=["name", "enabled", "is_primary", "valid_from", "valid_to"], order_by="valid_from asc, modified asc", limit_page_length=0)
...
def _save_branch_period(instructor: str, branch: str, start, end, *, enabled: int, make_primary: bool) -> dict:
    periods = _branch_periods(instructor, branch)
    exact = next((row for row in periods if core._same_date(row.valid_from, start) and core._same_date(row.valid_to, end)), None)
    if exact:
    ...
    overlapping = next((row for row in periods if cint(row.enabled) and core._overlap(start, end, row.valid_from, row.valid_to)), None)
    if overlapping:
...
def _ensure_academic_branch_access(instructor: str, row: PlannedAssignment) -> dict | None:
    if not row.enabled:
        return None
    return _save_branch_period(instructor, row.branch, row.valid_from, row.valid_to, enabled=1, make_primary=False)
...
def save_instructor_assignment_batch(payload: str | dict) -> dict:
    ...
    create, existing, conflicts = _classify(plan, meta["instructor"])
    if conflicts:
        frappe.throw(_("Instructor Assignment plan has {0} conflict(s). Resolve them before saving.").format(len(conflicts)), frappe.ValidationError)
    primary_exists = frappe.db.exists("EduEdge Instructor Branch Assignment", {"instructor": meta["instructor"], "is_primary": 1, "enabled": 1})
    explicit_results = []
    for row in branch_access:
        result = _save_branch_period(meta["instructor"], row.branch, row.valid_from, row.valid_to, enabled=row.enabled, make_primary=bool(not primary_exists and row.enabled))
