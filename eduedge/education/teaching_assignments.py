...
LEGACY_SUBJECT_TEACHER = "Subject Teacher"
...
UNIQUE_PRIMARY_ASSIGNMENT_TYPES = CLASS_RESPONSIBILITY_TYPES
...
def ensure_teaching_assignment_foundation() -> None:
    ...
    frappe.db.sql(
        """
        update `tabEduEdge Instructor Assignment`
        set assignment_type = %s
        where assignment_type = %s
        """,
        (SUBJECT_INSTRUCTOR, LEGACY_SUBJECT_TEACHER),
    )
