...
frappe.ui.form.on("EduEdge CBT Exam Schedule", {
	...
	exam_template(frm) {
		...
		frappe.db.get_value(
			...
			if (!message) return;
			const isPublic = message.exam_scope === "EduEdge Public Examination";
			const isBranchWide = message.template_reuse_scope === "Branch-wide";
			const isSpecificSubject = message.subject_applicability === "Specific Subject";
			frm.set_value({
				exam_scope: message.exam_scope,
				school_branch: isPublic ? null : isBranchWide ? message.school_branch : frm.doc.school_branch,
				course: isSpecificSubject ? message.course : frm.doc.course,
				student_group: isPublic ? null : isBranchWide ? message.student_group : null,
				academic_year: isPublic ? null : message.academic_year,
				academic_term: isPublic ? null : message.academic_term,
				assessment_group: isPublic ? null : message.assessment_group,
				maximum_assessment_score: 0,
	...
	assessment_plan(frm) {
		...
		frappe.db.get_value(
			...
			if (!message) return;
			// The Schedule's actual Branch, Subject, Class, and academic context remain authoritative.
			frm.set_value("maximum_assessment_score", message.maximum_assessment_score);
		});
	},
});
