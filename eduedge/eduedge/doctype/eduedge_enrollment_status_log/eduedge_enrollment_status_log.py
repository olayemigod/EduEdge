...
from eduedge.education.academic_progression import PROGRESSION_LEVEL_FIELD, progression_target
...
ALLOWED_TRANSITIONS = {
	"Active": {"Completed", "Promoted", "Repeated", "Deferred", "Withdrawn", "Suspended", "Held for Review", "Transferred", "Graduated", "Cancelled"},
	"Suspended": {"Active", "Deferred", "Withdrawn", "Held for Review", "Transferred", "Cancelled"},
	"Held for Review": {"Active", "Promoted", "Repeated", "Deferred", "Withdrawn", "Transferred", "Cancelled"},
	"Deferred": {"Active", "Withdrawn", "Transferred", "Cancelled"},
...
class EduEdgeEnrollmentStatusLog(Document):
	...
	def _validate_transition(self) -> None:
		...
		if self.new_status not in TARGET_STATUSES and (
			self.target_program_enrollment or self.target_program_offering or self.target_student_group
		):
			frappe.throw(_("A destination enrollment/group is only valid for promotion, repetition or transfer."), frappe.ValidationError)

	def _derive_target_context(self) -> None:
		if not self.target_program_enrollment:
			self.target_branch = None
			self.target_program_offering = None
			self.target_student_group = None
			return
		...
		self._validate_target_student_group(target, offering)
	...
	def _validate_target_transition(self, target, offering) -> None:
		...
		source_level = self._enrollment.get(PROGRESSION_LEVEL_FIELD) if self._enrollment.meta.has_field(PROGRESSION_LEVEL_FIELD) else None
		target_level = target.get(PROGRESSION_LEVEL_FIELD) if target.meta.has_field(PROGRESSION_LEVEL_FIELD) else None
		...
		if self.new_status == "Promoted":
			...
			if expected.get("terminal") or not expected.get("program"):
				frappe.throw(_("The source Programme/Level has no configured promotion target."), frappe.ValidationError)
			if target.program != expected.get("program") or target_level != expected.get("progression_level"):
				frappe.throw(_("Target enrollment does not match the configured next Class or Academic Level."), frappe.ValidationError)
			if not _is_later_academic_year(target.academic_year, self._enrollment.academic_year):
			...
			if not _is_later_academic_year(target.academic_year, self._enrollment.academic_year):
				frappe.throw(_("Repetition target must use a later Academic Session."), frappe.ValidationError)
		elif self.new_status == "Transferred":
			if source_institution and target_institution and target_institution != source_institution:
				frappe.throw(_("Automatic transfer is limited to Branches within the same Institution. Use Admissions for another Institution."), frappe.ValidationError)
			if target.program != self._enrollment.program or target_level != source_level:
				frappe.throw(_("Internal transfer must retain the same Programme/Class and Academic Level."), frappe.ValidationError)
			if target.get(OFFERING_FIELD) == self._enrollment.get(OFFERING_FIELD):
				frappe.throw(_("Transfer target must use another Programme Offering."), frappe.ValidationError)

	def _validate_target_student_group(self, target, offering) -> None:
		if not self.target_student_group:
			return
		group = frappe.get_doc("Student Group", self.target_student_group)
		group.check_permission("read")
		if group.disabled:
			frappe.throw(_("Target Class Arm / Group must be active."), frappe.ValidationError)
		if group.program != target.program or group.academic_year != target.academic_year:
			frappe.throw(_("Target Class Arm / Group must match the destination Programme and Academic Session."), frappe.ValidationError)
		if group.meta.has_field(OFFERING_FIELD) and group.get(OFFERING_FIELD) != offering.name:
			frappe.throw(_("Target Class Arm / Group must belong to the destination Programme Offering."), frappe.ValidationError)
		if group.meta.has_field(PROGRESSION_LEVEL_FIELD):
			group_level = group.get(PROGRESSION_LEVEL_FIELD)
			target_level = target.get(PROGRESSION_LEVEL_FIELD) if target.meta.has_field(PROGRESSION_LEVEL_FIELD) else None
			if group_level and group_level != target_level:
				frappe.throw(_("Target Class Arm / Group Academic Level does not match the destination enrollment."), frappe.ValidationError)


def _is_later_academic_year(target_year: str | None, source_year: str | None) -> bool:
