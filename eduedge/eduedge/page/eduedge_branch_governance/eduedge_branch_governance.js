...
frappe.pages["eduedge-branch-governance"].on_page_show = function (wrapper) {
	...
	frappe.require("edgeui.bundle.js", () => {
		...
		frappe.require("eduedge_branch_governance.bundle.js", () => {
			if (wrapper.current_visit_id !== visitId) return;
			if (!window.EduEdgeBranchGovernance) {
				fail(__("The EduEdge Branch Governance bundle is unavailable."));
				return;
			}
			$loading.remove();
			const root = $(
			...
			try {
				wrapper.vue_app = runtime.createEdgeApp(window.EduEdgeBranchGovernance, {
