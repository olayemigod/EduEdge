...
frappe.pages["eduedge-instructor-assignments"].on_page_show = function (wrapper) {
	...
	if (wrapper.vue_app) { try { wrapper.vue_app.unmount(); } catch (error) { console.error("Failed to unmount Teacher Assignments", error); } wrapper.vue_app = null; }
	$(page.body).empty();
	const $loading = $(`<div class="p-6 text-center text-muted">${__("Loading Teacher Assignments...")}</div>`).appendTo(page.body);
	const fail = (message) => { $loading.remove(); $(`<div class="alert alert-danger p-6 text-center"><strong>${__("Teacher Assignments failed to load")}</strong><div>${frappe.utils.escape_html(message || "")}</div></div>`).appendTo(page.body); };
	frappe.require("edgesuite_ui.bundle.js", () => {
		if (wrapper.current_visit_id !== visitId) return;
		frappe.require("eduedge_instructor_assignments.bundle.js", () => {
			if (wrapper.current_visit_id !== visitId) return;
			if (!window.EduEdgeInstructorAssignments || typeof window.createEduEdgeInstructorAssignmentsApp !== "function") return fail(__("The EduEdge Teacher Assignments bundle is unavailable."));
			$loading.remove();
			...
			catch (error) { console.error("Failed to mount Teacher Assignments", error); fail(error.message || String(error)); }
		});
	});
};
