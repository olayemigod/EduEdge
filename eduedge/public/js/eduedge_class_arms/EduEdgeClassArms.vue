		...
		<EdgePageLayout>
			<template #header>
				<EdgePageHeader
					...
					:subtitle="`Manage reusable ${classArmSingular.toLowerCase()} identities and their period-specific academic records. Teaching responsibility remains under Instructor Assignments.`"
			...
			<template v-else>
				...
				<section class="class-arm-layout">
					<article class="class-arm-panel">
						<div class="class-arm-heading"><div><p class="edge-eyebrow">Reusable identity + academic periods</p><h2>{{ classArmPlural }}</h2></div><button v-if="canCreate" type="button" class="edge-button" @click="newClassArm">New {{ classArmSingular }}</button></div>
						<EdgeLoadingState v-if="loading" :message="`Refreshing ${classArmPlural.toLowerCase()}...`" />
						<EdgeEmptyState v-else-if="!data.class_arms.length" :title="`No ${classArmPlural.toLowerCase()} found`" :description="canCreate ? `Create the first reusable ${classArmSingular.toLowerCase()} from a valid Programme Offering.` : 'Change the filters or contact an academic administrator.'" />
						<div v-else class="class-arm-list">
							<button v-for="row in data.class_arms" :key="row.name" type="button" class="class-arm-card" :class="{ 'is-selected': draft.name === row.name }" @click="editClassArm(row.name)">
								<div class="class-arm-title"><span><strong>{{ row.display_name || row.student_group_name || row.name }}</strong><small>{{ row.program || programmeSingular }} · {{ row.academic_year }}{{ row.academic_term ? ` · ${row.academic_term}` : "" }}</small></span><EdgeStatusBadge :label="row.disabled ? 'Disabled' : 'Active'" :status="row.disabled ? 'disabled' : 'active'" :tone="row.disabled ? 'danger' : 'success'" /></div>
								<div class="class-arm-meta"><span>{{ row.class_arm_identity?.class_arm_code || "Legacy identity" }}</span><span>{{ row.course || row.group_based_on || "Class" }}</span><span>{{ row.student_count || 0 }} {{ studentPlural.toLowerCase() }}</span><span>Teaching team via assignments</span></div>
							</button>
						</div>
					...
					<article class="class-arm-panel editor">
						<div class="class-arm-heading"><div><p class="edge-eyebrow">{{ draft.name ? "Academic period" : "Reusable identity + first period" }}</p><h2>{{ draft.name ? draft.display_name || classArmSingular : `New ${classArmSingular}` }}</h2></div><button type="button" class="edge-button" @click="newClassArm">Reset</button></div>
						...
						<template v-else>
							...
							<div v-if="draft.offering" class="class-arm-context">
								<div><span>Institution</span><strong>{{ contextInstitutionName }}</strong></div><div><span>{{ programmeSingular }}</span><strong>{{ draft.program || "Not resolved" }}</strong></div><div><span>{{ academicYearSingular }}</span><strong>{{ draft.academic_year || "Not resolved" }}</strong></div><div><span>{{ academicTermSingular }}</span><strong>{{ draft.academic_term || `${academicYearSingular}-wide` }}</strong></div><div><span>Reusable identity</span><strong>{{ draft.class_arm_identity?.class_arm_code || (draft.name ? "Migration required" : "Created on save") }}</strong></div><div><span>Previous period</span><strong>{{ draft.previous_student_group || "First known period" }}</strong></div>
							</div>

							<label><span>{{ classArmSingular }} Name</span><input v-model.trim="draft.display_name" class="form-control" :disabled="Boolean(draft.name)" :placeholder="`Example: JSS 1A or ${programmeSingular} Group A`" /><small v-if="draft.name">This is the reusable school-facing identity. Existing academic periods cannot be reassigned to a different identity.</small></label>
							...
							<section class="class-arm-roster">
								<div class="class-arm-subheading"><div><p class="edge-eyebrow">Period roster</p><h3>{{ studentPlural }}</h3></div><span>{{ draft.students.length }} selected</span></div>
								...
								<button v-if="draft.offering && !studentChoices.length" type="button" class="edge-button edge-button--primary" @click="openStudentEnrollments">Enroll Student</button>
							...
							<section v-if="draft.name && draft.class_arm_identity" class="rollover-panel">
								<div class="class-arm-subheading"><div><p class="edge-eyebrow">Period rollover</p><h3>Prepare Next Period</h3></div><span>Source remains unchanged</span></div>
								<p class="rollover-help">Create the next operational Student Group for the same reusable {{ classArmSingular }}. Only students with submitted enrollment in the destination Offering will carry forward.</p>
								<label><span>Destination {{ offeringSingular }}</span><select v-model="rollover.destination_offering" class="form-control" @change="resetRolloverPreview"><option value="">Select a later Offering</option><option v-for="offering in rolloverOfferings" :key="offering.name" :value="offering.name">{{ offering.offering_title || offering.name }} · {{ offering.academic_year }}{{ offering.academic_term ? ` · ${offering.academic_term}` : "" }}</option></select></label>
								<div class="class-arm-actions"><button type="button" class="edge-button" :disabled="!rollover.destination_offering || rollover.previewing" @click="previewRollover">{{ rollover.previewing ? "Checking..." : "Preview Rollover" }}</button><button v-if="rollover.preview && !rollover.preview.existing_student_group" type="button" class="edge-button edge-button--primary" :disabled="rollover.executing" @click="executeRollover">{{ rollover.executing ? "Preparing..." : "Prepare Next Period" }}</button></div>
								<p v-if="rollover.error" class="class-arm-error">{{ rollover.error }}</p>
								<div v-if="rollover.preview" class="rollover-summary">
									<div><span>Carry forward</span><strong>{{ rollover.preview.eligible_count }}</strong></div><div><span>Excluded</span><strong>{{ rollover.preview.excluded_count }}</strong></div><div><span>Status</span><strong>{{ rollover.preview.existing_student_group ? "Already prepared" : "Ready" }}</strong></div>
								</div>
								<p v-if="rollover.preview?.existing_student_group" class="rollover-help">This Class Arm already has a period for the selected Offering: {{ rollover.preview.existing_student_group }}</p>
								<div v-if="rollover.preview?.excluded_students?.length" class="rollover-exclusions"><strong>Not carried forward</strong><span v-for="student in rollover.preview.excluded_students" :key="student.name">{{ student.student_name || student.name }} · {{ student.reason }}</span></div>
							</section>
...
const emptyDraft = () => ({
	name: "", display_name: "", class_arm_identity: null, previous_student_group: "", branch: "", institution: "", offering: "", program: "", academic_year: "", academic_term: "", batch: "",
	group_based_on: "Batch", course: "", max_strength: 0, disabled: false, students: [], can_write: true,
});
const emptyOptions = () => ({ offerings: [], courses: [], students: [], class_arm_identities: [], context: {} });
...
const emptyRollover = () => ({ destination_offering: "", preview: null, previewing: false, executing: false, error: "" });

export default {
	name: "EduEdgeClassArms",
	data() { return { loading: true, loadedOnce: false, error: "", optionsLoading: false, saving: false, saveError: "", menuItems: EDUEDGE_MENU_ITEMS, filters: { branch: "", academic_year: "", academic_term: "", search: "" }, data: emptyData(), options: emptyOptions(), draft: emptyDraft(), rollover: emptyRollover(), studentSearch: "", initialCreateMode: false }; },
	computed: {
		...
		rolloverOfferings() { return (this.options.offerings || []).filter((row) => row.name !== this.draft.offering && (!this.draft.program || row.program === this.draft.program)); },
	},
	...
	methods: {
		...
		async newClassArm() { this.draft = { ...emptyDraft(), branch: this.filters.branch || this.data.selected_branch?.name || "", academic_year: this.filters.academic_year || "", academic_term: this.filters.academic_term || "" }; this.options = emptyOptions(); this.rollover = emptyRollover(); this.studentSearch = ""; this.saveError = ""; await this.loadOptions(); },
		async editClassArm(name) { this.saveError = ""; this.rollover = emptyRollover(); try { const response = await frappe.call("eduedge.api.class_arms.get_class_arm", { name }); const row = response.message || {}; this.draft = { ...emptyDraft(), ...row, offering: row.offering || "", branch: row.branch || this.filters.branch, disabled: Boolean(row.disabled), students: row.students || [] }; await this.loadOptions(); } catch (error) { this.saveError = error?.message || `${this.classArmSingular} could not be opened.`; } },
		...
		async draftBranchChanged() { this.filters.branch = this.draft.branch; this.draft.offering = ""; this.draft.institution = ""; this.draft.program = ""; this.draft.academic_year = ""; this.draft.academic_term = ""; this.draft.batch = ""; this.draft.course = ""; this.draft.students = []; await this.load(true); await this.loadOptions(); },
		async draftOfferingChanged() { this.draft.course = ""; this.draft.students = []; await this.loadOptions(); }, groupBasisChanged() { if (this.draft.group_based_on !== "Course") this.draft.course = ""; },
		...
		async saveClassArm() { if (!this.canSave) return; this.saving = true; this.saveError = ""; try { const response = await frappe.call({ method: "eduedge.api.class_arms.save_class_arm", type: "POST", args: { class_arm: this.draft.name || undefined, display_name: this.draft.display_name, branch: this.draft.branch, offering: this.draft.offering, group_based_on: this.draft.group_based_on, course: this.draft.course || undefined, max_strength: this.draft.max_strength || 0, disabled: this.draft.disabled ? 1 : 0, students: JSON.stringify(this.draft.students || []) } }); const saved = response.message || {}; frappe.show_alert({ message: __(`${this.classArmSingular} period saved`), indicator: "green" }); this.filters.branch = saved.branch || this.filters.branch; this.filters.academic_year = this.draft.academic_year || this.filters.academic_year; await this.load(true); if (saved.name) await this.editClassArm(saved.name); } catch (error) { this.saveError = error?.message || `${this.classArmSingular} could not be saved.`; } finally { this.saving = false; } },
		resetRolloverPreview() { this.rollover.preview = null; this.rollover.error = ""; },
		async previewRollover() { if (!this.draft.name || !this.rollover.destination_offering) return; this.rollover.previewing = true; this.rollover.error = ""; this.rollover.preview = null; try { const response = await frappe.call({ method: "eduedge.api.class_arms.preview_class_arm_rollover", type: "POST", args: { source: this.draft.name, destination_offering: this.rollover.destination_offering } }); this.rollover.preview = response.message || null; } catch (error) { this.rollover.error = error?.message || "Class Arm rollover could not be previewed."; } finally { this.rollover.previewing = false; } },
		async executeRollover() { if (!this.rollover.preview || !this.draft.name || !this.rollover.destination_offering) return; this.rollover.executing = true; this.rollover.error = ""; try { const response = await frappe.call({ method: "eduedge.api.class_arms.execute_class_arm_rollover", type: "POST", args: { source: this.draft.name, destination_offering: this.rollover.destination_offering } }); const result = response.message || {}; frappe.show_alert({ message: result.status === "existing" ? __("Class Arm period already exists") : __(`Prepared ${result.display_name || this.classArmSingular} for the next period`), indicator: result.status === "existing" ? "orange" : "green" }); await this.load(true); if (result.name) await this.editClassArm(result.name); } catch (error) { this.rollover.error = error?.message || "Class Arm rollover could not be completed."; } finally { this.rollover.executing = false; } },
...
<style scoped>
.class-arm-filter-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(12rem,1fr)); gap:.75rem; width:100%; }.class-arm-filter-grid label,.editor label,.rollover-panel label { display:grid; gap:.35rem; font-weight:600; }.class-arm-search { grid-column:span 2; }.class-arm-layout { display:grid; grid-template-columns:minmax(0,1fr) minmax(24rem,.95fr); gap:1rem; margin-top:1rem; }.class-arm-panel { display:grid; gap:1rem; align-content:start; padding:1rem; border:1px solid var(--border-color); border-radius:12px; background:var(--card-bg); }.class-arm-heading,.class-arm-title,.class-arm-subheading,.class-arm-actions,.class-arm-paging { display:flex; align-items:center; justify-content:space-between; gap:1rem; }.class-arm-heading h2,.class-arm-subheading h3 { margin:.2rem 0 0; }.class-arm-list,.choice-list,.class-arm-roster,.rollover-panel,.rollover-exclusions { display:grid; gap:.75rem; }.class-arm-card { display:grid; gap:.75rem; width:100%; padding:.9rem; text-align:left; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); }.class-arm-card:hover,.class-arm-card.is-selected { border-color:var(--primary); }.class-arm-title>span,.choice-row span { display:grid; gap:.15rem; }.class-arm-title small,.choice-row small,.editor small,.rollover-help { color:var(--text-muted); }.class-arm-meta { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:.5rem; color:var(--text-muted); font-size:.82rem; }.class-arm-context,.rollover-summary { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.75rem; padding:.8rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); }.class-arm-context>div,.rollover-summary>div { display:grid; gap:.15rem; }.class-arm-context span,.rollover-summary span { color:var(--text-muted); font-size:.78rem; }.two-column { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.75rem; }.class-arm-check { display:flex !important; align-items:center; gap:.5rem !important; font-weight:500 !important; }.class-arm-roster,.rollover-panel { padding-top:.75rem; border-top:1px solid var(--border-color); }.choice-list { max-height:18rem; overflow:auto; padding-right:.25rem; }.choice-row { display:grid; grid-template-columns:minmax(0,1fr) 6rem; align-items:center; gap:.75rem; padding:.65rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); }.choice-row>label { display:flex !important; align-items:center; gap:.6rem !important; font-weight:500 !important; }.rollover-exclusions { padding:.75rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); }.rollover-exclusions span { color:var(--text-muted); font-size:.85rem; }.class-arm-error { margin:0; color:var(--red-600,#b42318); } @media (max-width:1100px) { .class-arm-layout { grid-template-columns:1fr; } } @media (max-width:700px) { .class-arm-search { grid-column:auto; }.class-arm-meta,.class-arm-context,.rollover-summary,.two-column,.choice-row { grid-template-columns:1fr; }.class-arm-heading,.class-arm-title,.class-arm-subheading,.class-arm-actions,.class-arm-paging { align-items:stretch; flex-direction:column; } }
</style>
