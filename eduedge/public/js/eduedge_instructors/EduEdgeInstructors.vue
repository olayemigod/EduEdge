...
const blankInstructor = (institution = "", branch = "") => ({
	...
	assignments: [], branch_eligibility: [],
});
const blankData = () => ({
...
<style scoped>
.people-filters,.people-grid { display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); gap:.75rem; width:100%; }.people-filters label,.people-grid label { display:grid; gap:.35rem; font-weight:600; }.people-filters .wide,.people-grid .wide { grid-column:1/-1; }.people-layout { display:grid; grid-template-columns:minmax(20rem,.8fr) minmax(0,1.5fr); gap:1rem; margin-top:1rem; }.people-panel { display:grid; gap:1rem; align-content:start; padding:1rem; border:1px solid var(--border-color); border-radius:12px; background:var(--card-bg); }.people-heading,.actions,.profile-summary { display:flex; align-items:center; justify-content:space-between; gap:.75rem; flex-wrap:wrap; }.people-heading h2,.people-panel h3 { margin:0; }.people-list,.assignment-list { display:grid; gap:.65rem; }.person-card { display:grid; grid-template-columns:auto minmax(0,1fr) auto; align-items:center; gap:.75rem; padding:.75rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); text-align:left; }.person-card:hover,.person-card.is-selected { border-color:var(--primary); }.person-card span,.assignment-list article { display:grid; gap:.15rem; }.person-card small,.assignment-list small,.profile-summary small,.people-grid small { color:var(--text-muted); }.avatar,.photo { display:grid; place-items:center; overflow:hidden; border-radius:50%; border:1px solid var(--border-color); background:var(--card-bg); font-weight:700; }.avatar { width:2.8rem; height:2.8rem; }.photo { width:7rem; height:7rem; font-size:2rem; }.avatar img,.photo img { width:100%; height:100%; object-fit:cover; }.profile-summary { justify-content:flex-start; padding:1rem; border-radius:8px; background:var(--control-bg); }.profile-summary > div:nth-child(2) { display:grid; gap:.25rem; flex:1; }.assignment-list article { padding:.7rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); }.paging { display:flex; justify-content:space-between; align-items:center; }.people-error { color:var(--red-600,#b42318); } @media (max-width:1000px) { .people-layout { grid-template-columns:1fr; } } @media (max-width:680px) { .people-filters,.people-grid { grid-template-columns:1fr; }.people-filters .wide,.people-grid .wide { grid-column:auto; }.person-card { grid-template-columns:auto 1fr; }.person-card > :last-child { grid-column:1/-1; } }
</style>
