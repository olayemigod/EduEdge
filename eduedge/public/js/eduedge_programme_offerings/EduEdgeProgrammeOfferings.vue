				...
				<EdgeFilterBar title="Offering filters">
					<div class="eduedge-offering-filter-grid">
						...
						<label>
							...
							<select v-model="filters.department" class="form-control" @change="filterDepartmentChanged">
						...
						<label>
							<span>Status</span>
							<select v-model="filters.is_active" class="form-control" @change="applyFilters"><option value="">Active and disabled</option><option value="1">Active only</option><option value="0">Disabled only</option></select>
						</label>
...
export default {
	...
	computed: {
		...
		draftInstitutionName() { const branch = this.draftOptions.branches.find((row) => row.name === this.draft.school_branch); return branch?.institution_name || this.draft.institution || "Select a Branch"; },
	...
	methods: {
		...
		async load(resetStart = false) {
			...
			try {
				const response = await frappe.call("eduedge.api.programme_offerings.get_programme_offerings_page", { ...this.filters, start: this.data.paging.start || 0, page_length: this.data.paging.page_length || 25 });
		...
		filterBranchChanged() { this.filters.institution = ""; this.filters.department = ""; this.filters.program = ""; this.filters.student_batch = ""; this.newOffering(); this.load(true); },
		filterDepartmentChanged() { if (!this.filterProgrammes.some((row) => row.name === this.filters.program)) this.filters.program = ""; this.load(true); },
		filterYearChanged() { this.filters.academic_term = ""; this.load(true); },
