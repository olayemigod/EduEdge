			...
			<template v-else>
				<EdgeFilterBar title="Offering filters">
					<div class="eduedge-offering-filter-grid">
						...
						<label><span>Branch / Campus</span><select v-model="filters.branch" class="form-control" @change="filterBranchChanged"><option value="">All permitted Branches in Institution</option><option v-for="branch in filterBranches" :key="branch.name" :value="branch.name">{{ branch.branch_name || branch.name }}</option></select></label>
						<label><span>{{ departmentSingular }}</span><select v-model="filters.department" class="form-control" @change="filterDepartmentChanged"><option value="">All {{ departmentPlural.toLowerCase() }}</option><option v-for="department in filterDepartments" :key="department.name" :value="department.name">{{ department.department_name || department.name }}</option></select></label>
				...
				<section v-if="filterCalendar?.name" class="calendar-context-strip">
					<div><span>Resolved Institution Calendar</span><strong>{{ calendarTitle(filterCalendar) }}</strong></div>
					<div><span>Academic Session</span><strong>{{ filterCalendar.academic_year || filters.academic_year }}</strong></div>
					<div><span>Calendar Range</span><strong>{{ calendarRange(filterCalendar) }}</strong></div>
				</section>
				...
				<EdgeDashboardLayout min-column-width="12rem">
					...
					<EdgeStatCard label="Occupied Seats" :value="data.summary.occupied_seats || 0" helper="Submitted active/suspended enrollments" />
					<EdgeStatCard label="Configured Capacity" :value="data.summary.configured_capacity || 0" helper="Zero-capacity Offerings remain unlimited" />
				...
				<div class="eduedge-offering-layout">
						...
						<div v-else class="eduedge-offering-list">
							<article v-for="row in data.offerings" :key="row.name" class="eduedge-offering-card">
								<button type="button" class="eduedge-offering-main" @click="editOffering(row)">
									<div class="eduedge-offering-title">
										<span><strong>{{ row.offering_title }}</strong><small>{{ row.offering_code }}</small></span>
										<div class="eduedge-offering-badges">
											<EdgeStatusBadge :label="row.operational_status" :status="row.operational_status" :tone="statusTone(row.operational_status)" />
											<EdgeStatusBadge v-if="row.academic_term" label="Legacy Term Offering" status="legacy" tone="warning" />
											<EdgeStatusBadge :label="row.admission_status" :status="row.application_open ? 'open' : 'closed'" :tone="row.application_open ? 'success' : 'neutral'" />
											<EdgeStatusBadge :label="row.enrollment_status" :status="row.enrollment_open ? 'open' : 'closed'" :tone="row.enrollment_open ? 'success' : 'neutral'" />
										</div>
									</div>
									<div class="eduedge-offering-grid">
										<span><strong>{{ row.program }}</strong><small>{{ row.department || departmentSingular }} · {{ institutionName(row.institution) }} · {{ branchName(row.school_branch) }}</small></span>
										<span><strong>{{ row.academic_year }}</strong><small>{{ row.academic_term ? `Historical · ${row.academic_term}` : "Full Academic Session" }}</small></span>
										<span><strong>{{ row.study_mode }} · {{ row.delivery_mode }}</strong><small>{{ row.student_batch || "No cohort" }}</small></span>
										<span><strong>{{ capacityLabel(row) }}</strong><small>{{ row.occupied_seats }} occupied</small></span>
									</div>
								</button>
					...
					<section class="eduedge-offering-panel eduedge-offering-editor">
						<div class="eduedge-offering-heading"><div><p class="edge-eyebrow">{{ draft.name ? "Session Offering" : "New Session Offering" }}</p><h2>{{ draft.name ? draft.offering_title || editorOfferingSingular : `New ${editorOfferingSingular}` }}</h2></div><button type="button" class="edge-button" @click="newOffering">Reset</button></div>
						...
						<div v-else-if="draft.identity_locked" class="identity-warning"><strong>Offering identity is locked</strong><span>Applicants, Class Arms or submitted enrollments already reference this Offering. Create a new {{ editorOfferingSingular }} to change its Branch, {{ editorProgrammeSingular }}, Academic Session, cohort, study mode or delivery mode.</span></div>
						...
						<template v-else>
							<label><span>Institution</span><select v-model="draft.institution" class="form-control" :disabled="identityFieldsLocked" @change="draftInstitutionChanged"><option value="">Select Institution</option><option v-for="institution in editorInstitutions" :key="institution.name" :value="institution.name">{{ institution.institution_name || institution.name }}</option></select></label>
							<label><span>Branch / Campus</span><select v-model="draft.school_branch" class="form-control" :disabled="identityFieldsLocked || !draft.institution" @change="draftBranchChanged"><option value="">Select permitted Branch</option><option v-for="branch in draftBranches" :key="branch.name" :value="branch.name">{{ branch.branch_name || branch.name }}</option></select></label>
							<div class="eduedge-offering-context-readonly"><span>{{ editorDepartmentSingular }}</span><strong>{{ draftDepartmentName || `Derived from ${editorProgrammeSingular}` }}</strong></div>
							...
							<label><span>{{ editorAcademicYearSingular }}</span><select v-model="draft.academic_year" class="form-control" :disabled="identityFieldsLocked || !draft.institution" @change="draftYearChanged"><option value="">Select {{ editorAcademicYearSingular }}</option><option v-for="year in draftOptions.academic_years || []" :key="year.name" :value="year.name">{{ year.name }}</option></select><small>This Offering automatically covers all Terms / Semesters configured inside the selected Academic Session.</small></label>
							<div v-if="draftCalendar?.name" class="calendar-context-strip editor-calendar">
								<div><span>Resolved Institution Calendar</span><strong>{{ calendarTitle(draftCalendar) }}</strong></div>
								<div><span>Calendar Range</span><strong>{{ calendarRange(draftCalendar) }}</strong></div>
							</div>
							...
							<label><span>Student Batch / Cohort</span><select v-model="draft.student_batch" class="form-control" :disabled="identityFieldsLocked"><option value="">Not assigned</option><option v-for="batch in draftOptions.student_batches || []" :key="batch.name" :value="batch.name">{{ batch.name }}</option></select></label>
							<div class="eduedge-offering-two-column"><label><span>Study Mode</span><select v-model="draft.study_mode" class="form-control" :disabled="identityFieldsLocked"><option v-for="mode in draftOptions.study_modes || []" :key="mode" :value="mode">{{ mode }}</option></select></label><label><span>Delivery Mode</span><select v-model="draft.delivery_mode" class="form-control" :disabled="identityFieldsLocked"><option v-for="mode in draftOptions.delivery_modes || []" :key="mode" :value="mode">{{ mode }}</option></select></label></div>
							<label><span>{{ editorOfferingSingular }} Title</span><input v-model.trim="draft.offering_title" class="form-control" :disabled="legacyDraft" placeholder="Generated when left blank" /></label>
...
const emptyFilters = () => ({ branch: "", institution: "", department: "", program: "", academic_year: "", study_mode: "", delivery_mode: "", is_active: "", admission_enabled: "", enrollment_enabled: "", search: "" });
const emptyOptions = () => ({ institutions: [], branches: [], programmes: [], departments: [], academic_years: [], academic_terms: [], calendar_context: {}, student_batches: [], study_modes: [], delivery_modes: [] });
const emptyData = () => ({ active_context: {}, offerings: [], options: emptyOptions(), summary: { total_offerings: 0, visible_offerings: 0, active: 0, upcoming: 0, full: 0, occupied_seats: 0, configured_capacity: 0 }, paging: { start: 0, page_length: 25, has_more: false, next_start: 0 }, permissions: { can_create: false, can_write: false } });

export default {
	name: "EduEdgeProgrammeOfferings",
	data() { return { loading: true, loadedOnce: false, error: "", saving: false, saveError: "", menuItems: EDUEDGE_MENU_ITEMS, filters: emptyFilters(), draft: emptyDraft(), data: emptyData(), draftOptions: emptyOptions(), draftContext: {} }; },
	computed: {
		activeContext() { return this.data.active_context || {}; },
		programmeSingular() { return this.term("programme", false, "Programme / Class", this.activeContext); }, programmePlural() { return this.term("programme", true, "Programmes / Classes", this.activeContext); },
		departmentSingular() { return this.term("department", false, "Department / School Section", this.activeContext); }, departmentPlural() { return this.term("department", true, "Departments / School Sections", this.activeContext); },
		offeringSingular() { return this.term("programme_offering", false, "Programme Offering", this.activeContext); }, offeringPlural() { return this.term("programme_offering", true, "Programme Offerings", this.activeContext); },
		academicYearSingular() { return this.term("academic_year", false, "Academic Session", this.activeContext); }, academicYearPlural() { return this.term("academic_year", true, "Academic Sessions", this.activeContext); },
		editorProgrammeSingular() { return this.term("programme", false, "Programme / Class", this.draftContext || this.activeContext); },
		editorDepartmentSingular() { return this.term("department", false, "Department / School Section", this.draftContext || this.activeContext); },
		editorOfferingSingular() { return this.term("programme_offering", false, "Programme Offering", this.draftContext || this.activeContext); },
		editorAcademicYearSingular() { return this.term("academic_year", false, "Academic Session", this.draftContext || this.activeContext); },
		...
		identityFieldsLocked() { return Boolean(this.legacyDraft || this.draft.identity_locked); },
		...
		filterDepartments() { return this.data.options.departments || []; },
		filterProgrammes() { return (this.data.options.programmes || []).filter((row) => (!this.filters.institution || !row.eduedge_institution || row.eduedge_institution === this.filters.institution) && (!this.filters.department || row.department === this.filters.department)); },
		editorInstitutions() { return this.draftOptions.institutions?.length ? this.draftOptions.institutions : (this.data.options.institutions || []); },
		draftBranches() { return this.draft.institution ? (this.draftOptions.branches || []).filter((row) => row.institution === this.draft.institution) : []; },
		draftProgrammes() { return this.draft.institution ? (this.draftOptions.programmes || []).filter((row) => !row.eduedge_institution || row.eduedge_institution === this.draft.institution) : []; },
		draftDepartmentName() { const department = (this.draftOptions.departments || []).find((row) => row.name === this.draft.department); return department?.department_name || this.draft.department || ""; },
	...
	methods: {
		...
		calendarTitle(calendar) { const academicYear = calendar?.academic_year; return calendar?.calendar_title || (academicYear ? `${academicYear} Calendar` : calendar?.name || "Institution Academic Calendar"); },
		calendarRange(calendar) { const start = calendar?.start_date || calendar?.calendar_start_date; const end = calendar?.end_date || calendar?.calendar_end_date; return start && end ? `${start} – ${end}` : start || end || "Dates not configured"; },
		async load(resetStart = false, useActiveBranch = false) {
			...
			try {
				...
				if (!this.draft.institution) this.draft.institution = this.filters.institution || this.activeContext.institution || "";
				if (!this.draft.school_branch) this.draft.school_branch = this.filters.branch || this.activeContext.branch || "";
				if (!this.draftOptions.institutions?.length) this.draftOptions = { ...emptyOptions(), ...(this.data.options || {}) };
		...
		async loadDraftOptions() {
			try {
				const response = await frappe.call("eduedge.api.programme_offerings_safe.get_programme_offering_options", { institution: this.draft.institution || undefined, branch: this.draft.school_branch || undefined, academic_year: this.draft.academic_year || undefined });
				const result = response.message || {};
				this.draftOptions = { ...emptyOptions(), ...(result.options || {}) };
				this.draftContext = result.active_context || this.activeContext;
			} catch (error) { this.saveError = error?.message || "Programme Offering context could not be loaded."; }
		},
		applyFilters() { this.load(true, false); },
		async filterInstitutionChanged() { this.filters.branch = ""; this.filters.department = ""; this.filters.program = ""; this.filters.academic_year = ""; await this.load(true, false); },
		async filterBranchChanged() { this.filters.department = ""; this.filters.program = ""; this.filters.academic_year = ""; await this.load(true, false); },
		filterDepartmentChanged() { this.filters.program = ""; this.applyFilters(); },
		...
		async newOffering() { this.saveError = ""; this.draft = { ...emptyDraft(), institution: this.filters.institution || this.activeContext.institution || "", school_branch: this.filters.branch || this.activeContext.branch || "" }; await this.loadDraftOptions(); },
		async editOffering(row) { this.saveError = ""; this.draft = { ...emptyDraft(), ...row, is_active: Boolean(row.is_active), admission_enabled: Boolean(row.admission_enabled), enrollment_enabled: Boolean(row.enrollment_enabled), identity_locked: Boolean(row.identity_locked) }; await this.loadDraftOptions(); },
		async draftInstitutionChanged() { this.draft.school_branch = ""; this.draft.department = ""; this.draft.program = ""; this.draft.academic_year = ""; this.draft.academic_term = ""; this.draft.student_batch = ""; await this.loadDraftOptions(); },
		async draftBranchChanged() { this.draft.department = ""; this.draft.program = ""; this.draft.academic_year = ""; this.draft.academic_term = ""; this.draft.student_batch = ""; await this.loadDraftOptions(); },
...
<style scoped>
.eduedge-offering-filter-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(12rem,1fr));gap:.75rem;width:100%}.eduedge-offering-filter-grid label,.eduedge-offering-editor label{display:grid;gap:.35rem;font-weight:600}.eduedge-offering-search{grid-column:span 2}.session-rule,.calendar-context-strip{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.75rem;margin:1rem 0}.session-rule>div,.calendar-context-strip>div,.eduedge-offering-context-readonly{display:grid;gap:.2rem;padding:.8rem;border:1px solid var(--border-color);border-radius:8px;background:var(--control-bg)}.session-rule span,.calendar-context-strip span,.eduedge-offering-context-readonly span{color:var(--text-muted);font-size:.78rem}.editor-calendar{grid-template-columns:repeat(2,minmax(0,1fr));margin:0}.eduedge-offering-layout{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(20rem,.85fr);gap:1rem;margin-top:1rem}.eduedge-offering-panel{display:grid;gap:1rem;align-content:start;padding:1rem;border:1px solid var(--border-color);border-radius:12px;background:var(--card-bg)}.eduedge-offering-heading,.eduedge-offering-title{display:flex;justify-content:space-between;align-items:flex-start;gap:1rem}.eduedge-offering-heading h2{margin:0}.eduedge-offering-list{display:grid;gap:.75rem}.eduedge-offering-card{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.75rem;align-items:center;padding:.85rem;border:1px solid var(--border-color);border-radius:8px;background:var(--control-bg)}.eduedge-offering-main{display:grid;gap:.75rem;width:100%;padding:0;border:0;background:transparent;color:inherit;text-align:left}.eduedge-offering-title>span,.eduedge-offering-grid span,.eduedge-offering-context-readonly{display:grid;gap:.2rem}.eduedge-offering-title small,.eduedge-offering-grid small,.eduedge-offering-editor small{color:var(--text-muted)}.eduedge-offering-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:.75rem}.eduedge-offering-badges,.eduedge-offering-editor-actions,.eduedge-offering-checks{display:flex;flex-wrap:wrap;gap:.5rem}.eduedge-offering-two-column{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.75rem}.eduedge-offering-checks label{display:flex;align-items:center;gap:.4rem;font-weight:500}.eduedge-offering-paging{display:flex;justify-content:space-between;align-items:center;gap:.75rem}.legacy-warning,.identity-warning{display:grid;gap:.25rem;padding:.75rem;border:1px solid var(--orange-300,#f4b860);border-radius:8px;background:var(--control-bg)}.eduedge-offering-error{margin:0;color:var(--red-600,#b42318)}@media(max-width:1100px){.eduedge-offering-layout{grid-template-columns:1fr}.eduedge-offering-grid,.session-rule,.calendar-context-strip{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:700px){.eduedge-offering-search{grid-column:auto}.eduedge-offering-card,.eduedge-offering-grid,.eduedge-offering-two-column,.session-rule,.calendar-context-strip{grid-template-columns:1fr}.eduedge-offering-heading,.eduedge-offering-title,.eduedge-offering-paging{align-items:stretch;flex-direction:column}}
</style>
