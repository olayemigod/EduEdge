			...
			<template v-else>
					...
					<div class="eduedge-programme-filter-grid">
						...
						<label>
							...
							<input
								...
								@input="queueDepartmentSearch(filters.department)"
								@change="applyFilters"
							/>
						</label>
						<label>
				...
				<EdgeDashboardLayout min-column-width="12rem">
					...
					<EdgeStatCard
						...
						:value="data.summary.unclassified_visible"
						helper="Visible legacy records without Institution"
						:tone="data.summary.unclassified_visible ? 'warning' : 'neutral'"
					/>
				</EdgeDashboardLayout>
				...
				<div class="eduedge-programme-layout">
					<section class="eduedge-programme-panel">
						<div class="eduedge-programme-panel-heading">
							<div>
								<p class="edge-eyebrow">Catalogue</p>
								<h2>{{ programmePlural }}</h2>
							</div>
						...
						<EdgeEmptyState
							v-else-if="!data.programmes.length"
							:title="`No ${programmePlural.toLowerCase()} found`"
							description="Change the filters or create the first record for this Institution."
						/>
						<div v-else class="eduedge-programme-list">
							<article v-for="row in data.programmes" :key="row.name" class="eduedge-programme-row">
								<button type="button" class="eduedge-programme-main" @click="editProgramme(row)">
									<span class="eduedge-programme-title">
										<strong>{{ row.program_name || row.name }}</strong>
										<small>{{ row.program_abbreviation || row.name }}</small>
									</span>
									<span class="eduedge-programme-context">
										{{ institutionName(row.eduedge_institution) }}
										<small>{{ sectionName(row.eduedge_academic_section) || "No academic section" }}</small>
									</span>
									<span class="eduedge-programme-context">
										{{ row.department || "No department" }}
										<small>{{ formatDate(row.modified) }}</small>
									</span>
						...
						<div class="eduedge-programme-paging">
							<button type="button" class="edge-button" :disabled="loading || data.paging.start <= 0" @click="previousPage">
								Previous
							</button>
							<span>
								{{ data.paging.start + (data.programmes.length ? 1 : 0) }}–{{ data.paging.start + data.programmes.length }}
								of {{ data.summary.total_programmes }}
							</span>
							<button type="button" class="edge-button" :disabled="loading || !data.paging.has_more" @click="nextPage">
								Next
							</button>
						</div>
					</section>
					...
					<section class="eduedge-programme-panel eduedge-programme-editor">
						<div class="eduedge-programme-panel-heading">
							<div>
								...
								<h2>{{ draft.name ? draft.program_name || programmeSingular : `New ${programmeSingular}` }}</h2>
							</div>
						...
						<EdgeEmptyState
							...
							description="Your current role can view these records but cannot create or edit them."
						/>
						<template v-else>
							<label>
								<span>{{ programmeSingular }} name</span>
								<input v-model.trim="draft.program_name" class="form-control" />
							</label>
							<label>
								<span>Abbreviation</span>
								<input v-model.trim="draft.program_abbreviation" class="form-control" />
							</label>
							<label>
								<span>Institution</span>
							...
							<label>
								...
								<select v-model="draft.eduedge_academic_section" class="form-control">
									...
									<option v-for="section in draftSections" :key="section.name" :value="section.name">
										{{ section.section_name }}
									</option>
								</select>
							</label>
							<label>
								...
								<input
									...
									@input="queueDepartmentSearch(draft.department)"
								/>
							</label>
							<p class="text-muted">
								Course rows, portal settings, and advanced fields remain in the full Program form.
							</p>
							<div class="eduedge-programme-editor-actions">
								<button
									...
									@click="saveProgramme"
								>
									{{ saving ? "Saving..." : `Save ${programmeSingular}` }}
								</button>
								<button
									v-if="draft.name"
									type="button"
									class="edge-button"
									@click="openFullForm(draft.name)"
								>
									Open full form
								</button>
							</div>
...
export default {
	...
	data() {
		return {
			...
			data: {
				...
				summary: {
					total_programmes: 0,
					visible_programmes: 0,
					course_rows: 0,
					active_offerings: 0,
					unclassified_visible: 0,
				},
				paging: {
					start: 0,
					page_length: 25,
	...
	computed: {
		...
		sectionSingular() {
			return this.term("academic_section", false, "Academic Section");
		},
		sectionPlural() {
			return this.term("academic_section", true, "Academic Sections");
		},
		canCreate() {
		...
		draftSections() {
			return this.data.sections.filter(
				(section) => section.institution === this.draft.eduedge_institution
			);
		},
	},
	mounted() {
	...
	methods: {
		...
		institutionName(name) {
			return (
				this.data.institutions.find((row) => row.name === name)?.institution_name ||
				name ||
				"Unclassified Institution"
			);
		},
		sectionName(name) {
		...
		async load(resetStart = false) {
			...
			try {
				...
				if (!this.draft.eduedge_institution) {
					this.draft.eduedge_institution =
						this.filters.institution || this.activeContext.institution || "";
				}
				this.loadedOnce = true;
			} catch (error) {
				this.error = error?.message || `${this.programmePlural} could not be loaded.`;
			} finally {
				this.loading = false;
			}
		},
		...
		draftInstitutionChanged() {
			const valid = this.draftSections.some(
				(section) => section.name === this.draft.eduedge_academic_section
			);
		...
		async saveProgramme() {
			...
			try {
				...
				frappe.show_alert({
					message: __(`${this.programmeSingular} saved`),
					indicator: "green",
				});
				await this.load(true);
				const row = this.data.programmes.find((item) => item.name === saved.name);
				if (row) this.editProgramme(row);
				else this.newProgramme();
			} catch (error) {
				this.saveError = error?.message || `${this.programmeSingular} could not be saved.`;
			} finally {
				this.saving = false;
			}
		},
		queueDepartmentSearch(value) {
			if (this.departmentTimer) window.clearTimeout(this.departmentTimer);
			this.departmentTimer = window.setTimeout(() => this.loadDepartments(value), 250);
		},
		async loadDepartments(value) {
			try {
				const response = await frappe.call("eduedge.api.programmes.search_departments", {
					txt: value || undefined,
				});
		...
		openFullForm(name) {
			if (!name) return;
			frappe.set_route("Form", "Program", name);
		},
		openNativeList() {
			window.open("/app/program", "_blank", "noopener,noreferrer");
		},
	},
};
</script>
...
.eduedge-programme-filter-grid label,
.eduedge-programme-editor label {
	display: grid;
	gap: 0.35rem;
	font-weight: 600;
}

.eduedge-programme-layout {
	display: grid;
...
.eduedge-programme-panel-heading {
	display: flex;
...
.eduedge-programme-panel-heading h2 {
	margin: 0;
}

.eduedge-programme-list {
	display: grid;
	gap: 0.75rem;
}

.eduedge-programme-row {
	display: grid;
...
.eduedge-programme-main {
	display: grid;
	grid-template-columns: minmax(12rem, 1.2fr) minmax(10rem, 1fr) minmax(9rem, 0.9fr) minmax(12rem, auto);
	gap: 0.75rem;
	align-items: center;
	width: 100%;
	padding: 0;
...
.eduedge-programme-title,
.eduedge-programme-context {
	display: grid;
	gap: 0.2rem;
}

.eduedge-programme-title small,
.eduedge-programme-context small {
	color: var(--text-muted);
}

.eduedge-programme-counts,
.eduedge-programme-editor-actions {
	display: flex;
	flex-wrap: wrap;
	gap: 0.5rem;
}

.eduedge-programme-paging {
	display: flex;
	justify-content: space-between;
	align-items: center;
	gap: 0.75rem;
