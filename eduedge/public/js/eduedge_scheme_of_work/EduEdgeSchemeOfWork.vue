		...
		<EdgePageLayout>
			<template #header>
				<EdgePageHeader
					...
					subtitle="Plan term curriculum against the exact Branch, sessional Class Offering, Class Arm and Subject responsibility. Approved versions retain readable curriculum snapshots."
			...
			<template v-else>
				<EdgeFilterBar title="Academic context">
					<div class="scheme-filters">
						...
						<label><span>Class / Programme Offering</span><select v-model="filters.program_offering" class="form-control" @change="offeringChanged"><option value="">All Classes</option><option v-for="row in data.offerings" :key="row.value" :value="row.value">{{ row.label }} · {{ row.academic_year }}{{ row.academic_term ? ` · Historical ${row.academic_term}` : '' }}</option></select></label>
						<label><span>Term / Semester</span><select v-model="filters.academic_term" class="form-control" :disabled="!filters.program_offering" @change="termChanged"><option value="">{{ filters.program_offering ? 'Select Term / Semester' : 'Select Class first' }}</option><option v-for="row in data.terms || []" :key="row.value" :value="row.value">{{ row.label }}</option></select></label>
				...
				/>
				<EdgeActionBar
					v-if="filters.program_offering && !filters.academic_term"
					label="Programme Offering and Class Arm are session-wide. Select a Term / Semester from the Institution Academic Calendar to plan curriculum delivery for that period."
				/>
				...
				<section class="scheme-layout">
					<article class="scheme-panel register">
						...
						<EdgeEmptyState v-else-if="!data.schemes.length" title="No Scheme of Work found" description="Select an Academic Term, Class context and Subject to prepare the first Scheme for this period." />
					...
					<article class="scheme-panel editor">
						...
						<EdgeEmptyState v-if="!draft.name && !canCreate" title="Select a Scheme or complete the filters" description="Choose Branch, Class, Term / Semester and Subject. Instructor options remain limited to the exact assignment context." />
						<template v-else>
							<div class="context-summary">
								...
								<div><span>Term / Semester</span><strong>{{ draft.academic_term || filters.academic_term || '—' }}</strong></div>
								<div><span>Academic Period</span><strong>{{ draft.period_start_date || selectedTerm?.start_date || '—' }} → {{ draft.period_end_date || selectedTerm?.end_date || '—' }}</strong></div>
							...
							<div v-if="draft.status === 'Approved' || draft.status === 'Retired'" class="snapshot-summary">
								...
								<span>{{ draft.course_name_snapshot || courseLabel(draft.course) }} · {{ draft.offering_title_snapshot || offeringLabel(draft.program_offering) }}{{ draft.student_group_name_snapshot ? ` · ${draft.student_group_name_snapshot}` : '' }} · {{ draft.academic_term || 'Historical period' }}</span>
							...
							<EdgeEmptyState v-if="!draft.items.length" title="No curriculum delivery items" description="Add Topics in the order they should be taught during the selected academic period." />
...
const blankData = () => ({
	allowed_branches: [], offerings: [], terms: [], groups: [], courses: [], topics: [], schemes: [], selected_term: null,
...
const blankScheme = (filters = {}, offering = null, term = null) => ({
	...
	academic_year: offering?.academic_year || "", academic_term: filters.academic_term || offering?.academic_term || "",
	period_start_date: term?.start_date || offering?.period_start_date || "", period_end_date: term?.end_date || offering?.period_end_date || "",
...
export default {
	...
	data() {
		return {
			...
			data: blankData(),
			filters: { school_branch: "", program_offering: "", academic_term: "", student_group: "", course: "", status: "", start: 0 },
	...
	computed: {
		...
		selectedTerm() { return (this.data.terms || []).find((row) => row.value === this.filters.academic_term) || this.data.selected_term || null; },
		selectedCourse() { return this.data.courses.find((row) => row.value === this.filters.course) || null; },
		canCreate() { return Boolean(this.data.permissions?.can_create_in_context && this.filters.program_offering && this.filters.academic_term && this.filters.course); },
		editable() { return Boolean((!this.draft.name || this.draft.status === "Draft") && (this.canCreate || this.data.permissions?.is_manager)); },
		canSave() { return Boolean(this.editable && this.draft.program_offering && this.draft.academic_term && this.draft.course); },
	},
	mounted() { this.load(); },
	methods: {
		...
		async load(reset = false, selectedName = "") {
			...
			try {
				const response = await frappe.call("eduedge.api.scheme_of_work_workbench_sessional.get_scheme_workbench", {
		...
		async branchChanged() {
			this.filters.program_offering = ""; this.filters.academic_term = ""; this.filters.student_group = ""; this.filters.course = ""; this.draft = blankScheme();
			await this.load(true);
		},
		async offeringChanged() {
			this.filters.academic_term = ""; this.filters.student_group = ""; this.filters.course = ""; this.draft = blankScheme();
			await this.load(true);
		},
		async termChanged() { this.draft = blankScheme(); await this.load(true); },
		async groupChanged() {
			this.filters.course = ""; this.draft = blankScheme();
			await this.load(true);
		},
		async courseChanged() { this.draft = blankScheme(); await this.load(true); },
		newScheme() { if (this.canCreate) { this.saveError = ""; this.draft = blankScheme(this.filters, this.selectedOffering, this.selectedTerm); } },
		async editScheme(row) {
			this.filters.program_offering = row.program_offering || "";
			this.filters.academic_term = row.academic_term || "";
		...
		async save() {
			...
			try {
				const payload = {
					...this.draft,
					school_branch: this.filters.school_branch,
					program_offering: this.filters.program_offering,
					academic_term: this.filters.academic_term,
					student_group: this.filters.student_group,
					course: this.filters.course,
				};
				const response = await frappe.call({ method: "eduedge.api.scheme_of_work_sessional.save_scheme", type: "POST", args: { payload: JSON.stringify(payload) } });
				this.draft = { ...blankScheme(), ...(response.message || {}) };
		...
		async runAction(method, message) {
			...
			try {
				...
				this.filters.status = "";
				this.filters.academic_term = this.draft.academic_term || this.filters.academic_term;
				await this.load(true, this.draft.name);
...
<style scoped>
.scheme-filters { display:grid; grid-template-columns:repeat(6,minmax(0,1fr)); gap:.65rem; width:100%; }.scheme-filters label,.item-grid label,.notes { display:grid; gap:.3rem; font-weight:600; }.scheme-layout { display:grid; grid-template-columns:minmax(18rem,.72fr) minmax(0,1.6fr); gap:1rem; margin-top:1rem; }.scheme-panel { display:grid; gap:1rem; align-content:start; padding:1rem; border:1px solid var(--border-color); border-radius:12px; background:var(--card-bg); }.scheme-heading,.scheme-actions { display:flex; align-items:center; justify-content:space-between; gap:.65rem; flex-wrap:wrap; }.scheme-heading h2,.scheme-heading h3 { margin:0; }.scheme-list,.scheme-items { display:grid; gap:.65rem; }.scheme-card { display:grid; grid-template-columns:minmax(0,1fr) auto; gap:.7rem; align-items:center; padding:.75rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); text-align:left; }.scheme-card.is-selected,.scheme-card:hover { border-color:var(--primary); }.scheme-card span { display:grid; gap:.18rem; }.scheme-card small,.snapshot-summary small,.topic-snapshot small { color:var(--text-muted); }.context-summary { display:grid; grid-template-columns:repeat(4,minmax(0,1fr)); gap:.65rem; }.context-summary div,.snapshot-summary { display:grid; gap:.2rem; padding:.65rem; border:1px solid var(--border-color); border-radius:8px; background:var(--control-bg); }.context-summary span { font-size:.78rem; color:var(--text-muted); }.scheme-item { position:relative; padding:.8rem; border:1px solid var(--border-color); border-radius:9px; background:var(--control-bg); }.item-grid { display:grid; grid-template-columns:repeat(3,minmax(0,1fr)); gap:.65rem; }.item-grid .wide { grid-column:1/-1; }.item-grid .topic-field { grid-column:span 1; }.scheme-item .remove { margin-top:.6rem; }.topic-snapshot { margin-top:.6rem; }.paging { display:flex; justify-content:space-between; align-items:center; }.scheme-error { color:var(--red-600,#b42318); }.notes { margin-top:.25rem; } @media (max-width:1200px) { .scheme-filters { grid-template-columns:repeat(3,minmax(0,1fr)); }.scheme-layout { grid-template-columns:1fr; } } @media (max-width:700px) { .scheme-filters,.context-summary,.item-grid { grid-template-columns:1fr; }.item-grid .wide,.item-grid .topic-field { grid-column:auto; } }
</style>
