...
class TestAcademicFoundationOperationsAlignmentContract(unittest.TestCase):
	...
	def test_readiness_uses_effective_calendar_without_false_missing_error(self):
		...
		display = (APP / "api" / "academic_foundation_display.py").read_text()
		...
		self.assertIn("base.get_academic_foundation", display)
		self.assertIn('"eduedge.api.academic_foundation.get_academic_foundation": "eduedge.api.academic_foundation_display.get_academic_foundation"', hooks)
	...
	def test_programme_offerings_use_only_institution_sessions_and_native_programmes(self):
		...
		display = (APP / "api" / "programme_offerings_display.py").read_text()
		...
		self.assertIn("annotate_link", display)
		self.assertIn("draftProgramChanged", component)
		self.assertIn("program_display_name", component)
	...
	def test_student_group_and_schedule_validate_native_calendar_and_programme_context(self):
		...
		attendance = (APP / "education" / "attendance_validation.py").read_text()
		...
		self.assertIn("More than one Course Schedule exists", attendance)

	def test_operations_fail_closed_and_explain_calendar_block(self):
		review = (APP / "api" / "academic_operations_review.py").read_text()
		display = (APP / "api" / "academic_operations_display.py").read_text()
		...
		self.assertIn("base.get_operations_context", display)
