...
class TestAcademicOperationsPreQASecurity(unittest.TestCase):
	def test_legacy_unclassified_masters_fail_closed_for_scoped_users(self):
	...
	def test_teacher_scope_resolves_user_employee_instructor_and_owned_academic_records(self):
		...
		self.assertIn('_owned_student_condition("`tabStudent`.name", resolved_user)', permissions)
		self.assertIn('_owned_student_condition("`tabProgram Enrollment`.student", resolved_user)', permissions)
	...
	def test_academic_operations_ui_hides_unauthorised_actions(self):
		...
		for token in (
			...
			"canReadRooms",
			"canReadInstructorAssignments",
			"canManageAttendance",
			"canSubmitAttendance",
			"frappe.datetime?.get_today?.()",
			"this.register.course_schedule?.name",
		):
			self.assertIn(token, component)
		self.assertIn(':disabled="student.locked || saving || !canManageAttendance"', component)

	def test_offering_context_does_not_accept_unclassified_links(self):
		controller = (
			APP
			/ "eduedge"
			/ "doctype"
		...
		self.assertIn("Assign the selected Programme to an Institution", controller)
		self.assertIn("Student Batch / Cohort must belong to the same Institution", controller)
		self.assertIn("Select an enabled Academic Level", controller)
