...
class TestAcademicOperationsPreQASecurity(unittest.TestCase):
	...
	def test_native_attendance_enforces_schedule_identity_and_serialised_duplicate_check(self):
		...
		self.assertIn("def _resolve_exact_attendance_schedule", operations)
		self.assertIn('"student_group": doc.student_group, "schedule_date": doc.date', operations)
		self.assertIn("limit_page_length=2", operations)
		self.assertIn("doc.course_schedule = matches[0].name", operations)
		self.assertIn("More than one Course Schedule exists for this Class and date", operations)
