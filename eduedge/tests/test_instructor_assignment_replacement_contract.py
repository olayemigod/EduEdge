...
class TestInstructorAssignmentReplacementContract(unittest.TestCase):
    ...
    def test_replacement_preserves_branch_eligibility_and_requires_upstream_coverage(self):
        source = (APP / "api" / "instructor_assignment_replacement.py").read_text(encoding="utf-8")
        self.assertIn("assignment_eligibility_preview(incoming.name, source.school_branch", source)
        self.assertIn("assert_instructor_branch_eligibility(", source)
        self.assertIn('"type": "branch-eligibility-blocked"', source)
