...
class TestProgrammeOfferingsPageContract(unittest.TestCase):
	...
	def test_offering_save_uses_document_validation_platform_guard_and_link_permissions(self):
		api = (APP / "api" / "programme_offerings_safe.py").read_text(encoding="utf-8")
		self.assertIn('require_eduedge_access(feature_key="academics", action="save_programme_offering")', api)
		...
		self.assertIn("assert_institution_calendar_context", api)
	...
	def test_department_filters_programmes_and_programme_derives_department(self):
		...
		self.assertIn("filterProgrammes", component)
		self.assertIn("filterDepartmentChanged", component)
		self.assertIn("draftProgramChanged", component)
		self.assertIn('this.draft.department = programme?.department || ""', component)
	...
	def test_page_loader_uses_canonical_edgesuite_bundle(self):
		...
		self.assertLess(loader.index("edgesuite_ui.bundle.js"), loader.index("eduedge_programme_offerings.bundle.js"))
	...
	def test_ci_checks_programme_offerings_entries(self):
		...
		self.assertIn("node --check eduedge/eduedge/page/eduedge_program_offerings/eduedge_program_offerings.js", workflow)
